...
; Turn DPCM_SAFE on to read the controllers twice and discard the
; read values if they match, to work around the occasional serial
; bit deletion.  Turn it off if using peripherals that malfunction
; if extra latch pulses are applied, such as a Super NES Mouse, or
; if you're not using DPCM.
.ifndef DPCM_SAFE_PADS
DPCM_SAFE_PADS = 1
.endif
